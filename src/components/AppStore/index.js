...
class AppStore extends Component {
  state = {
    searchInput: '',
    userClickedTabId: tabsList[0].tabId,
  }

  setActiveTabId = tabId => {
    this.setState({userClickedTabId: tabId})
  }
  ...
  getActiveTabApps = searchedApps => {
    const {userClickedTabId} = this.state
    const filteredApps = searchedApps.filter(
      eachSearchedApp => eachSearchedApp.category === userClickedTabId,
    )
  ...
  render() {
    const {searchInput, userClickedTabId} = this.state
          ...
          <ul className="tabs-list">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={userClickedTabId === eachTab.tabId}
              />
            ))}
          </ul>
